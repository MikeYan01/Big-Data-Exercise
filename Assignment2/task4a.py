...
# total trips
total_trips = all_trip.map(lambda x: (x[16], 1))
total_trips = total_trips.reduceByKey(lambda x, y: x + y)

# total revenue
total_revenue = all_trip.map(lambda x: (x[16], float(x[5])))
total_revenue = total_revenue.reduceByKey(lambda x, y: x + y)

# total tip percentage
total_tip_percentage = all_trip.map(lambda x: (x[16], 0 if float(x[5]) == 0 else float(x[8]) / float(x[5])))
